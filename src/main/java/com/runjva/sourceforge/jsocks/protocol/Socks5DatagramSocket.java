...
public class Socks5DatagramSocket extends DatagramSocket {
    ...
    private static final Logger log = Logger.getLogger(Socks5DatagramSocket.class.getName());
    ...
    public Socks5DatagramSocket(SocksProxyBase p, int port, InetAddress ip)
        ...
        log.fine("Datagram Socket:" + getLocalAddress() + ":" + getLocalPort());
        log.fine("Socks5Datagram: " + relayIP + ":" + relayPort);
    }
    ...
    public void send(DatagramPacket dp) throws IOException {
        ...
        if (!server_mode && proxy.isDirect(dp.getAddress())) {
            super.send(dp);
            log.fine("Sending datagram packet directly:");
            return;
        }
        ...
        if (proxy != null) {
            try {
                ...
                if (eof < 0) {
                    ...
                    log.warning("This really should not happen");
